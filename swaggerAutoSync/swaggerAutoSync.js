...
export default class ProjectInterfaceSync extends Component {
  ...
  async getSyncData() {
    ...
    if (result.data.errcode === 0) {
      if (!result.data.data) {
        this.state.sync_data.is_sync_open = false;
  ...
  // 是否开启
  onChange = v => {
    this.state.sync_data.is_sync_open = v;
    this.setState(this.state);
  };
          ...
          <div>
            ...
            <FormItem {...formItemLayout} label={<span>类cron风格表达式(默认每分钟更新一次)&nbsp;<a href="https://github.com/node-schedule/node-schedule">参考</a></span>}>
              {getFieldDecorator('sync_cron', {
                ...
                initialValue: this.state.sync_data.sync_cron ? this.state.sync_data.sync_cron : '30 * * * * *'
              })(<Input />)}
            </FormItem>
          </div>
